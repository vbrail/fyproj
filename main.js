...
//===================================================================================
 global.Detain = function( cur_sem, enroll  ){
   var data = Students.findById(enroll);
   //////////  incomplete
   return false;///for trail
        ...
        mongoData.forEach(ele =>{                                                    //-6
          Students.findById(ele['Enrollment No.'],function(err,data){                //-7
            if(err){                                                                 //-8
              ...
              if(!data){                                                             //-10
                console.log("no pre existing id",ele['Enrollment No.']);
              }                                                                      //10-
              else{                                                                  //-11
        ...
        mongoData.forEach(ele =>{
          Students.findById(ele['Enrollment No.'],function(err,data){
            if(err){
              ...
              if(!data){
                console.log("no pre existing id",els['Erollment No.']);
              }
              else{
                console.log("inserting fee data");
                var _id    = ele['Enrollment No.'],
...
app.get("/data",function(req,res){
  console.log("get data page initiated")
  res.render("data.ejs");
})
